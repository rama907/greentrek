...
function sendEmail($to_email, $to_name, $subject, $body_html) {
    ...
    try {
        ...
        // Kredensial SMTP dibaca dari config.php (lihat config.example.php)
        // Contoh untuk Gmail, SendGrid, Mailgun, dll.
        $mail->isSMTP();                                            // Kirim menggunakan SMTP
        $mail->Host       = (defined('SMTP_HOST') ? SMTP_HOST : 'smtp.gmail.com');                     // Ganti dengan SMTP host Anda (e.g., smtp.gmail.com)
        $mail->SMTPAuth   = true;                                   // Aktifkan autentikasi SMTP
        $mail->Username   = (defined('SMTP_USERNAME') ? SMTP_USERNAME : '');               // Ganti dengan username email Anda
        $mail->Password   = (defined('SMTP_PASSWORD') ? SMTP_PASSWORD : '');                  // Ganti dengan password email Anda (atau App Password jika pakai Gmail)
        ...
        $mail->setFrom(defined('MAIL_FROM') ? MAIL_FROM : (defined('SMTP_USERNAME') ? SMTP_USERNAME : ''), defined('MAIL_FROM_NAME') ? MAIL_FROM_NAME : 'GreenTrek'); // Alamat email pengirim dan nama
